...
- 復帰時に必要な「次の一手」「確認待ち」「問題」「進捗」「決定」「発見」「検証」などだけをAIが選び、出所のプロジェクトとセッションを付けて最大12件に整理
- 5秒未満の誤操作と、対応エージェントの活動も退席前メモもない区間は履歴に保存しない
- 要約担当CLIは収集元と無関係に選択
...
表示言語はmacOSの「優先する言語」の先頭に合わせます。日本語と英語に対応し、その他の言語では英語へフォールバックします。システム言語を変更した場合はCapsStackを再起動すると反映されます。
